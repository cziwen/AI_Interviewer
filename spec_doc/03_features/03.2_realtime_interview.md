...
- ✅ **独立决策层**：候选人每次说完后先经文本 LLM 输出流程动作，再由后端映射为下一轮 Turn
- ✅ **Server VAD**：自动检测说话起止，无需手动操作
- ✅ **智能追问**：AI 根据回答质量自动追问，确保每道主问题（包括最后一题）都遵循 `followup_limit`
- ✅ **回答有效性门槛 (Answer Gate)**：在结束面试前校验最后一题回答质量，防止因 AI 软性话术或用户短句导致过早收尾
...
在候选人说话结束（`speech_stopped`）后，后端不会立即进入旧规则分支，而是先走一次独立文本 LLM 决策调用。

```mermaid
flowchart TD
    speechStopped[SpeechStopped] --> buildCtx[BuildDecisionContext]
    buildCtx --> callDecision[CallIndependentTextLLM]
    callDecision --> decisionOk{DecisionValidWithinTimeout}
    decisionOk -->|yes| mapPlan[MapActionToTurnPlan]
    decisionOk -->|no| legacyPlan[LegacyRulePlanner]
    mapPlan --> sendTurn[send_response_create_with_turn]
    legacyPlan --> sendTurn
```

- **输入上下文**：当前阶段、当前题、候选人最新发言、最近 2-4 轮摘要。
- **输出动作**：`followup` / `next_question` / `answer_candidate_question` / `clarify` / `finish_interview`（受 `allowed_actions` 动态约束）。
- **结束保护**：当 `main_questions_completed < main_count_target` 时，服务端会阻断 `finish_interview`，强制继续流程（优先进入下一主问题）。
- **稳定性策略**：决策超时（默认 800ms）或 JSON 解析失败时，自动回退 legacy planner，面试不中断。
