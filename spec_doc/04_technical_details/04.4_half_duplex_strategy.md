...
判断：currentTime (2.0) < nextStartTimeRef (2.3) + 0.2
...
**0.2 秒缓冲**可以覆盖这些误差，确保扬声器完全停止后才开启麦克风。
...
```
[MIC] isAgentSpeaking = false  (候选人可以说话)
[WS] Event: response.created
[TTS] Pre-locking mic for 1.5s
[MIC] isAgentSpeaking = true   (门控生效)
[TTS] response.audio.delta chunk #1
[TTS] Scheduling playback at 2.5
[MIC] isAgentSpeaking = true   (持续门控)
...
[TTS] Scheduling playback at 5.8
[MIC] isAgentSpeaking = true
[MIC] isAgentSpeaking = true
...
| 候选人说话被截断 | 缓冲时间太长 | 减少 0.2s 缓冲到 0.1s |
