...
- 当前项目使用 `server_vad + create_response = false`，由后端显式发送 `response.create` 来控制每一轮 AI 发言
- 如果需要理解当前编排混乱点、turn 生命周期和推荐重构方案，请参考 [Realtime Turn 编排重构规范](04.5_realtime_turn_orchestrator_refactor.md)

### 用户语音转写（input_audio_transcription）

Session 中配置 `input_audio_transcription: { "model": "whisper-1" }` 后，服务端会对**用户输入音频**进行转写，并下发事件：

- **`conversation.item.input_audio_transcription.completed`**：某段用户音频转写完成，payload 中含 `item_id` 与 `transcript`。

后端在 [backend/app/api/realtime.py](../../backend/app/api/realtime.py) 中监听该事件，将 `transcript` 写入对话 Log（`*_dialogue.log`）并存入 Orchestrator 的 `user_transcripts`；保存对应 `Answer` 时优先使用该 transcript，从而避免面试结束后再调用 Whisper 做一次 STT。详见 [日志系统](../../05_logging.md) 与 [AI 评估](../03_features/03.3_ai_evaluation.md)。
...
在候选人 `speech_stopped` 后，后端会先发起一次独立文本 LLM 决策调用（不复用语音 Realtime 会话），用于判定流程动作。该调用只消费精简上下文：当前阶段、当前题、最新发言、最近 2-4 轮摘要。

- 决策输出格式固定为 JSON：
```json
{
  "action": "followup | next_question | answer_candidate_question | clarify | finish_interview",
  "reason": "简要说明"
}
```
- 决策层会收到 `allowed_actions`、`main_questions_completed`、`main_count_target`、`remaining_main_questions` 等结构化约束；模型返回动作必须在 `allowed_actions` 内。
- 当 `remaining_main_questions > 0` 时，prompt 与解析层都禁止 `finish_interview`，并在非法动作时回退 legacy planner。
- 服务端存在硬约束：即使模型返回 `finish_interview`，当 `main_questions_completed < main_count_target` 时也会阻断并改道到继续流程（优先推进下一主问题）。
- 系统使用 `asyncio.wait_for` 做超时控制（默认 `800ms`）。
- 如发生超时、JSON 解析失败、action 非法或 API 异常，立即回退到 legacy 规则规划器，面试流程不中断。
- 决策结果会映射为 `TurnPlan` 再进入既有的 `send_response_create_with_turn()` 与 turn lifecycle。
...
候选人若一直不开口，VAD 不会触发 `speech_stopped`，AI 不会自动回复。后端在每次 `response.done` 后启动定时器（默认 18 秒）；若在时间内未收到 `input_audio_buffer.speech_started`，则发送 `response.create`，指令为「候选人尚未回答。请简短重复当前问题或礼貌提醒候选人作答，不要换题。」一旦检测到 `speech_started` 即取消定时器。详见 [实时语音面试 - 长时间无回答时重新提问](../03_features/03.2_realtime_interview.md#41-长时间无回答时重新提问)。
...
wscat -c "wss://api.openai.com/v1/realtime?model=gpt-realtime-mini" \
...
- [Realtime Turn 编排重构规范](04.5_realtime_turn_orchestrator_refactor.md)
