...
| `silence_duration_ms` | 200-2000 | 600 | 持续静音多久后判定说话结束 |
...
```python
    ...
    # 注意：在 server_vad 模式下，OpenAI 会自动：
    # - commit buffer
    # - create response
    # 不需要手动发送这些指令
```
...
sequenceDiagram
    ...
    Note over RT: 自动 commit + 自动 create response
...
| 值 | 效果 | 适用场景 |
|----|------|---------|
| 400-500ms | 快速响应，但可能截断句子 | 节奏紧凑的快问快答 |
| 600ms | 默认值，适合正常对话节奏 | 大多数面试场景 |
| 800-1000ms | 容忍更长停顿，避免误判 | 候选人思考时间较长 |
...
"turn_detection": {
    "type": "server_vad",
    "threshold": 0.5,
    "silence_duration_ms": 900  # 增加容忍时间
}
```
